...
{// it recieves a dictionnaryList 
	if (index<0 || index >= sizeM){
		cout<< "index is out of bounds"<<endl;
	}
	cursorM = headM;
	i=0;
	while (cursorM!=0 && i< index){
...
ostream& operator <<(ostream& os, const DictionaryList &x){
	x.cursorM = x.headM; // initialize the cursor to the headM
	if(x.cursorM == nullptr){
		os<< "The dictionary list is empty."<<endl;
		return os;
	}
	while (dictList.cursorM != nullptr) {
        os << x.cursorM->keyM<<" "<<x.cursorM->datumM.charsM; // Output the charsM 
        x.cursorM = dictList.cursorM->next; // Move to the next node
        
        // Formatting: Add a newline after each entry
        if (dictList.cursorM != nullptr) {
